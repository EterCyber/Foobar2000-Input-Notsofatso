...
int CNSFFile::LoadFile_NSFE(service_ptr_t<file> p_filehint, abort_callback &p_abort, BYTE needdata)
{
	...
	while (!bEndFound)
	{
		if (p_filehint->is_eof(p_abort))		break/*return -1*/;
		...
		//char a[80] = { 0 };
		//memcpy(a, &nChunkType, 4);
		//pfc::string8 s = a;

		//MessageBoxA(0, s.c_str(), "switch (nChunkType)", 0);
		...
		case CHUNKTYPE_INFO:
			...
			p_filehint->seek_ex(nChunkSize - nChunkUsed, file::t_seek_mode::seek_from_current, p_abort);
		...
		case CHUNKTYPE_DATA:
			...
			p_filehint->seek_ex(nChunkSize, file::t_seek_mode::seek_from_current, p_abort);
		...
		case CHUNKTYPE_TIME:
			...
			p_filehint->seek_ex(nChunkSize - (nChunkUsed * 4), file::t_seek_mode::seek_from_current, p_abort);
	...
	//MessageBoxA(0, "1", "switch (nChunkType)", 0);

	if (needdata)
	{
		p_filehint->seek_ex(nDataPos, file::t_seek_mode::seek_from_beginning, p_abort);
